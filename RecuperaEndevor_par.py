...
Uso:
    ...
    RecuperaEndevor_par.py <ELEMENT>
...
Subir WORKERS es una decision sobre carga en un host compartido, no
una optimizacion local. Consultalo antes de pasarte de 4.
...
def execute(cfg: Config, elements: list[str]) -> tuple[list[list[str]], int]:
    """PARALELO: cfg.workers consultas concurrentes. pool.map preserva el
    orden del input, asi que el contador sigue siendo coherente."""
    def task(element: str):
        return element, *run_zowe(build_args(cfg, element), cfg.timeout)

    rows: list[list[str]] = []
    failed = 0
    total = len(elements)
    with ThreadPoolExecutor(max_workers=cfg.workers) as pool:
        for n, (element, rc, out) in enumerate(pool.map(task, elements), 1):
            if rc != 0:
